...
void decode_can_frame(const can_frame& frame, ros::Publisher& pub) {
    if (frame.can_id == 0x80000586) {
        if (frame.data[7] == 0x00) {
            ...
            imu_msg.orientation.x = static_cast<double>(roll);
            imu_msg.orientation.y = static_cast<double>(pitch);
            imu_msg.orientation.z = static_cast<double>(yaw);
            ...
            // Add your logic to handle accelerometer data
            ...
            // Add your logic to handle gyroscope data
        } else {
